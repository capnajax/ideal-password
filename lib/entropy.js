...
const	ENTROPY_CLASSES = [
  ...
  { score: Math.log(40), name: "bopomofo", range: [0x31A0,0x31ba]},
...
function passwordEntropy(_password) {
  ...
  let hasIllegalCharacter = false; // means at least one character is not known
  let result = {
    classes: [], 
    classNames: [],
  ...
  let foundClass = function(ecj) {
    if (!entropyClassnamesFound.has(ecj.name)) {
      entropyClassnamesFound.add(ecj.name);
  ...
  result.legal = !result.classes.find(e => e.name === 'illegal');
  ...
  for (i = 0; i < result.classes.length; i++) {
    if (cancelations.has(result.classes[i].name)) {
      continue;
    }
    result.entropy += result.classes[i].score;
    result.classNames.push(result.classes[i].name)
  }
